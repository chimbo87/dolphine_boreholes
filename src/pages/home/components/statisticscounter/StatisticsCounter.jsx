...
const StatisticsCounter = () => {
  ...
  useEffect(() => {
    ...
    const intervals = {
      experience: setInterval(() => {
        setCounters((prev) => ({
          ...prev,
          experience: Math.min(
            prev.experience + Math.ceil(finalValues.experience / steps),
            finalValues.experience
          ),
        }));
      }, stepDuration),
      ...
      projects: setInterval(() => {
        setCounters((prev) => ({
          ...prev,
          projects: Math.min(
            prev.projects + Math.ceil(finalValues.projects / steps),
    ...
    const checkCompletion = setInterval(() => {
      if (
        counters.experience >= finalValues.experience &&
      ...
      <div className="max-w-6xl mx-auto px-3">
        <div className="text-center mb-8">
          ...
          <p className="text-base text-gray-600 max-w-xl mx-auto">
            With decades of experience in borehole drilling and water resource
            management, we deliver results you can trust.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {stats.map((stat, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`inline-flex items-center justify-center w-12 h-12 rounded-full bg-opacity-10 mb-4 ${stat.color.replace(
                    "text-",
                    "bg-"
                  )}`}
                >
